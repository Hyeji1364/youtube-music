    ...
    return (
        <>
            ...
            <section className='youtube-result'>
                <h3><span className='icon3'></span>유튜브 검색 결과입니다. 음악을 듣거나 리스트에 추가할 수 있습니다.</h3>
                <ul>
                    {searchResults.map((result, index) => (
                        <li key={index}>
                            <span className='img' style={{ backgroundImage: `url(${result.imageURL})` }}></span>
                            <span className='title'>{result.title}</span>
                            <span className='platNow'><span className='icon4'></span></span>
                            <span className='listAdd'><span className='icon4'></span></span>
                        </li>
                    ))}
                </ul>
            </section>
